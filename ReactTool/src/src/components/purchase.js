...
const handleProductFormSubmit = async (e) => {
  ...
  const handleCreateProducts = async () => {
    try {
      ...
      if (!formData.name || !formData.primaryBarcode || !formData.purchasePrice || !formData.defaultSalesPrice) {
        alert('Please fill in all required fields');
        return;
      }
  
      // Prepare the primary product data
      const primaryProductData = {
        ...
        primaryBarcode: formData.primaryBarcode,
        purchasePrice: formData.purchasePrice,
        defaultSalesPrice: formData.defaultSalesPrice,
        silverPrice: formData.silverPrice || formData.defaultSalesPrice,
        goldPrice: formData.goldPrice || formData.defaultSalesPrice,
        platinumPrice: formData.platinumPrice || formData.defaultSalesPrice,
        diamondPrice: formData.diamondPrice || formData.defaultSalesPrice,
        warehouse: formData.primaryWarehouse,
        type: 'primary'
      };
  
      // If secondary barcode exists, add those details
      if (formData.hasSecondaryBarcode) {
        primaryProductData.secondaryBarcode = formData.secondaryBarcode;
        primaryProductData.secondaryBarcodeStock = formData.secondaryBarcodeStock;
        primaryProductData.secondaryBarcodeWarehouse = formData.secondaryBarcodeWarehouse;
      }
  
      // Create an array to hold all products to be created
      const productsToCreate = [primaryProductData];
  
      // If kaccha version is needed, add kaccha product data
      if (formData.hasKacchaVersion) {
        const kacchaProductData = {
          name: `${formData.name} (Kaccha)`,
          primaryBarcode: `K${formData.primaryBarcode}`, // Prefix 'K' for kaccha products
          purchasePrice: formData.kacchaFormData.purchasePrice,
          defaultSalesPrice: formData.kacchaFormData.defaultSalesPrice,
          silverPrice: formData.kacchaFormData.silverPrice || formData.kacchaFormData.defaultSalesPrice,
          goldPrice: formData.kacchaFormData.goldPrice || formData.kacchaFormData.defaultSalesPrice,
          platinumPrice: formData.kacchaFormData.platinumPrice || formData.kacchaFormData.defaultSalesPrice,
          diamondPrice: formData.kacchaFormData.diamondPrice || formData.kacchaFormData.defaultSalesPrice,
          warehouse: formData.kacchaFormData.warehouse,
          type: 'kaccha',
          linkedToPrimary: formData.primaryBarcode // Link to primary product
        };
  
        productsToCreate.push(kacchaProductData);
      }
  
      // Send the data to the backend
      const response = await axios.post('http://127.0.0.1:5000/batch-create-products', {
      ...
      if (status==='success') {
        alert('Product(s) created successfully!');
          ...
          {purchaseItems.map((item, index) => (
            <TableRow key={index}>
              ...
              <TableData>
                <Input
                  type="text"
                  value={item.barcode_1}
                  onChange={(e) => handleItemChange(index, 'barcode_1', e.target.value)}
                  onBlur={() => handleBarcodeSearch(item.barcode_1, index)}
...
<button type="submit" onClick={handleCreateProducts}>Create Product</button>
</StyledForm>
