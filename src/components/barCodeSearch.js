...
const BarcodeSearch = () => {
  ...
  const handleInvoiceTypeChange = (e) => {
    ...
    // Reset selections when changing away from hybrid
...
// 
  ...
  useEffect(() => {
    ...
    if (location.state && location.state.invoiceData && location.state.invoiceData.products) {
      ...
      // console.log(invoiceData.customer_name);
...
const handleSearchChange = (e) => {
  ...
  const handleSearch = async () => {
    try {
      ...
      const foundProduct = rows.find((row) => row[1] === barcode);
